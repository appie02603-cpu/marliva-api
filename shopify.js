module.exports = async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      `https://${store}/admin/api/2024-01/orders.json?status=any&created_at_min=${since}&limit=250&fields=id,name,total_price,financial_status,line_items`,
    ...
    orders.forEach(o => (o.line_items || []).forEach(li => {
      prodMap[li.title] = (prodMap[li.title] || 0) + parseFloat(li.price || 0) * li.quantity;
    }));
    const topProds = Object.entries(prodMap).sort((a, b) => b[1] - a[1]).slice(0, 5);
    const recentOrders = orders.slice(0, 6).map(o => ({
      name: o.name, total: parseFloat(o.total_price || 0), status: o.financial_status
    }));
